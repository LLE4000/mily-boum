...
/* LE RYTHME EST PROPRE À L'ÎLE, depuis que deux cartes ont un ciel qui
   gronde. Les ténèbres roulent DEUX FOIS PLUS SOUVENT que la jungle —
   demandé, et cohérent : sous une canopée la pluie occupe déjà
   l'oreille, tandis qu'un monde de lave n'a que le ciel pour faire du
   bruit. C'est une fonction et non une constante parce que l'ÉCLAIRAGE
   et le SON la lisent tous les deux : s'ils ne comptaient pas les
   mêmes cycles, on verrait un éclat sans l'entendre, ou l'inverse. */
function periodeRoulement(){
  return (typeof jeu !== "undefined" && jeu && typeof carteTornades === "function"
          && carteTornades(jeu.index)) ? MET_ROULEMENT * 0.5 : MET_ROULEMENT;
}
...
function roulementJungle(tps){
  var P = periodeRoulement();
  var n = Math.floor(tps / P);
  ...
  var d = tps - n * P - (0.4 + bruitStable(n, 0) * Math.max(0.3, P - 1.4));
...
function majTonnerre(dt, tps){
  greffeSonJungle();
  if(typeof son === "undefined" || !son.ok()) return;
  var n = Math.floor(tps / periodeRoulement());
