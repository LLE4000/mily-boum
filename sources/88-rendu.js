...
function dessineEffet(c, e, tps){
  ...
  if(e.t === "boum"){
    ...
    var sup = e.sup ? 1 : 0;
    c.save();
    /* LE FLASH SE COMPTE EN SECONDES, PAS EN FRACTION DE DURÉE — et
       c'est un vrai correctif, pas un réglage.
       Il s'écrivait « t < 0,16 », or t vaut age/duree : sur une Nova
       qui vit 3,2 s, cela faisait UNE DEMI-SECONDE d'écran blanc. On
       ne voyait donc ni le champignon, ni les anneaux, ni rien de ce
       pour quoi l'effet existe — la capture est sans appel, toute
       l'image reste laiteuse jusqu'à un tiers de seconde. Et le défaut
       s'aggravait tout seul : allonger la durée de la super Nova
       ALLONGEAIT son flash.
       Un éclair de bombe dure un dixième de seconde. On le lit donc
       sur e.age, qui est en secondes, et il ne dépend plus de rien
       d'autre. La super flashe un peu moins fort : elle occupe déjà
       trois fois plus d'écran, elle n'a pas besoin d'effacer le reste
       par-dessus. */
    var af = e.age;
    if(af < 0.13){
      c.fillStyle = "rgba(255,255,245," + (1 - af / 0.13) * (sup ? 0.60 : 0.80) + ")";
      c.fillRect(-W, -H, W * 3, H * 3);
    }
    c.globalCompositeOperation = "lighter";
    /* LA BOULE DE FEU NE SUIT PAS LE RAYON DE LA SUPER, et c'est le
       correctif qui a sauvé cet effet. e.r triple avec la super ; la
       boule, elle, est un dégradé PLEIN — à 13,8 cases elle faisait
       cinq cent quatre-vingts pixels de rayon et noyait toute l'image
       dans du blanc. Ce qui doit grandir sur une super Nova, c'est ce
       qui est CREUX : les anneaux et le champignon. Ce qui est plein
       reste mesuré. */
    var rFeu = sup ? e.r * 0.42 : e.r;
    /* la boule enfle en un tiers de seconde, en SECONDES elle aussi :
       sur t, son gonflement s'étalait sur plus d'une seconde et elle
       restait pleine et pâle tout ce temps */
    var rb = rFeu * RX * z * (0.4 + Math.min(1, af / 0.33) * 1.5);
...
/* ---------------------------------------------------------------
   ...
   --------------------------------------------------------------- */
/* ================================================================
   OÙ L'ON PEUT DÉBARQUER — LA BANDE DE PLAGE, TRACÉE

   « Lorsqu'on débarque, est-ce que la zone où on peut débarquer les
     troupes pourrait avoir un trait noir bien visible ? Comme ça on
   ...
   bande sert, et jamais après. Elle ne dépend donc pas d'un état
   d'interface de plus : elle lit le seul fait qui compte, `jeu.barges`.

   ET ELLE RESPIRE À L'INSTANT DU CHOIX. Changer de navette rallume la
   bande une seconde — c'est la réponse au geste, celle qui dit « oui,
   celle-ci, et voilà où elle ira ». Passé la seconde, elle redescend au
   trait discret : une bande qui clignoterait sans fin sur toute la
   longueur de l'île finirait par se regarder au lieu de se lire.
   ================================================================ */
var bargeSelVue = -1, bargeSelT = 0;
function dessineBandeDebarquement(c, tps){
  if(!jeu || jeu.fin || !jeu.barges || !jeu.barges.length) return;
  if(typeof modeApercu !== "undefined" && modeApercu
     && !(typeof modeEssai !== "undefined" && modeEssai)) return;
  /* le battement du choix : il se déclenche au CHANGEMENT, pas à
     chaque image, sinon il ne dirait rien */
  ...
  /* LE SABLE SE FONCE UN PEU — « ou alors toute cette zone-là se remet
     un petit peu plus foncé ». Assez pour détacher la bande du reste de
     la plage, pas assez pour la salir. */
  c.beginPath();
  c.moveTo(a.x, a.y); c.lineTo(b.x, b.y); c.lineTo(d.x, d.y); c.lineTo(e.x, e.y);
  c.closePath();
  c.fillStyle = "rgba(18,12,26," + (0.13 + 0.10 * vif).toFixed(3) + ")";
  c.fill();
  /* LE TRAIT NOIR, et il est plafonné en pixels d'écran : ce bloc est
     peint dans le repère MONDE, où une épaisseur fixe s'évanouit au
     dézoom — or c'est de loin qu'on vise la plage. */
  c.lineJoin = "round";
  c.strokeStyle = "rgba(12,8,18," + (0.62 + 0.28 * vif).toFixed(3) + ")";
  c.lineWidth = Math.max(2.0 / cam.z, 2.6);
  c.stroke();
  /* et un liseré clair par-dessus, pour qu'il tienne sur la lave comme
     sur le sable clair */
  c.strokeStyle = "rgba(255,236,196," + (0.30 + 0.34 * vif).toFixed(3) + ")";
  c.lineWidth = Math.max(0.8 / cam.z, 1.1);
  c.stroke();
  c.restore();
}
...
function dessineZonesSol(c, tps){
  var i;
  /* La bande de débarquement passe SOUS tout le reste des zones : c'est
     un repère de terrain, pas un effet. */
  dessineBandeDebarquement(c, tps);
