...
/* ================================================================
   ...
   transporte.
   ================================================================ */
var IBI_PAVE   = 2.6;      // le pas du carrelage cuit dans le sol
...
function paveLaPiste(){
  var cx = [], cy = [], allee = [], rayon = [], damier = [], bouge = [];
  ...
  for(i = 0; i < n; i++){
    for(j = 0; j < n; j++){
      ...
      if(!dansLaScene(gx, gy)) continue;
      var dx = gx - SCENE_GX, dy = gy - SCENE_GY;
      var r = Math.sqrt(dx * dx + dy * dy);
      /* sous le podium, rien ne se voit : autant ne pas le peindre */
      if(r < IBI_DEMI + 0.4) continue;
      var q = iso(gx, gy);
...
function dessinePisteIbiza(c, tps){
  ...
  for(i = 0; i < D.n; i++){
    var g = gainDalle(D.rayon[i], D.damier[i], H, f, u, reg) * F * D.bouge[i];
  ...
  c.save();
  /* DÉCOUPÉ À L'ÉTOILE : une dalle du bord dépasse de la piste, et
     c'est l'arête de l'étoile qui fait la figure de cette île. */
  c.beginPath();
  ...
  for(var b = 0; b < IBI_SEAUX.length; b++){
    ...
    c.globalAlpha = Math.min(0.92, al * 0.62);
