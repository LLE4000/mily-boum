...
function majUnitesDistantes(j, p){
  ...
  for(i = 0; i < p.length; i++){
    var e = p[i];
    if(!j.unites[i]){
      j.unites[i] = { gx:e[0], gy:e[1], cx:e[0], cy:e[1], type:"furie", droite:true,
                      phase:Math.random() * 6, fr:1 };
    }
    var u = j.unites[i];
    u.cx = e[0]; u.cy = e[1];
    var code = e[2] | 0;
    ...
    u.droite = !!(code & 1);
    /* LA SANTÉ, QUAND ELLE EST DITE. Un quatrième élément absent veut
       dire « intacte » — c'est le cas d'une troupe en pleine forme
       comme celui d'un client d'une version antérieure, et les deux
       méritent exactement la même réponse : pas de jauge. */
    u.fr = (e.length > 3) ? borne((e[3] | 0) / 15, 0, 1) : 1;
  }
  ...
  if(reseau.etatT <= 0){
    ...
    if(jeu){
      ...
      for(var i = 0; i < n && p.length < EQ.UNITES_DIFFUSEES; i += pas){
        ...
        if(it < 0) it = 0;
        /* ════════════════════════════════════════════════════════════
           LA SANTÉ, EN QUATRIÈME ÉLÉMENT

           « Est-ce qu'on pourrait voir la santé des troupes des autres,
             la petite jauge au-dessus de leur tête ? »

           ELLE NE POUVAIT PAS ENTRER DANS LE TROISIÈME. Le code y porte
           le type et l'orientation, et le destinataire y lit `code >> 1`
           borné à la table des troupes : y ajouter des bits hauts aurait
           fait lire à toute version antérieure un indice de type énorme,
           donc ramené au dernier — toutes les troupes du salon changées
           de silhouette d'un coup. Un CHAMP DE PLUS, à la fin, est au
           contraire ignoré sans dommage : c'est le même patron que `sq`,
           `pu` et `c` avant lui.

           ET IL N'EST ÉCRIT QUE SI LA TROUPE EST BLESSÉE. Une unité
           intacte n'envoie rien du tout, exactement comme le faisait un
           client d'avant — d'où deux propriétés d'un coup : le message
           ne grossit pas tant que personne n'a été touché, et « absent »
           veut dire « intacte » aussi bien pour une vieille version que
           pour une troupe en pleine forme. Il n'y a donc rien à
           distinguer à la lecture.
           Quinze crans : la jauge fait vingt pixels de large, on n'y
           verrait pas le seizième.
           LE DÉNOMINATEUR EST `pvMax`, PAS LA FICHE. Une troupe partie
           avec le bonus de garde d'une relique a plus de vie que sa
           fiche n'en annonce : mesurée sur la fiche, elle serait
           partie à cent pour cent et le serait restée longtemps après
           avoir été touchée. C'est le même `pvMax` que lit la jauge de
           mes propres troupes. ═══════════════════════════════════════ */
        var e15 = Math.round((u.pv / (u.pvMax || u.pv || 1)) * 15);
        e15 = Math.max(0, Math.min(15, e15));
        var ligne = [Math.round(u.gx * 10) / 10, Math.round(u.gy * 10) / 10,
                     (it << 1) + (u.droite ? 1 : 0)];
        if(e15 < 15) ligne.push(e15);
        p.push(ligne);
      }
